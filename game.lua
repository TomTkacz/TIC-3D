...
function Object3D._renderRoutines.mesh(self)
    data = scene.loadedObjects[self.meshID]
    for i=2,#data do
        face=data[i]
        faceData={}
        for _,vertex in pairs(face) do
            vertexPos = Pos3D(table.unpack(vertex))
            screenPos=worldSpaceToScreenSpace()
            table.insert(faceData,screenPos.x)
            table.insert(faceData,screenPos.y)
        end
        table.insert(faceData,12)
        printTable(faceData)
        trib(table.unpack(faceData))
    end
end
	...
	while bytesOffset < MAP_SIZE_BYTES do
		...
		for i=1,12 do
			if peek(curByteAddr()) ~= 0 then
				meshName = meshName..byteToUTF8(peek(curByteAddr()))
			end
