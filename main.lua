...
function TIC()
	...
	if t==0 then
		loadObjects()
		cube=Object3D("mesh","cube",Pos3D(0,0,0))
	end

	cls(0)

	if btn(0) then scene.get(cube).origin=scene.get(cube).origin-Pos3D(0,0,0.1) end
	if btn(1) then scene.get(cube).origin=scene.get(cube).origin+Pos3D(0,0,0.1) end
	if btn(2) then scene.get(cube).origin=scene.get(cube).origin+Pos3D(0.1,0,0) end
	if btn(3) then scene.get(cube).origin=scene.get(cube).origin-Pos3D(0.1,0,0) end
